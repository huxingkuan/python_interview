...
1. is和==的区别？
   ...
   is用于判断两个变量引用的对象是否是同一个，==用于判断两个变量的值是否相等。a is b 相当于id(a) == id(b)，id() 能够获取对象的内存地址，但当a=[1, 2, 3]，b=a[:]时，虽然a和b的值是一样的，但因为切片返回的是一个新的对象，所以两者的内存地址不同，因此返回False。如果定义a=10，b=10，然后再比较a is b会发现返回的是True，这是因为在Python中会创建一个小型的整型池子，范围为[-5, 256]，为这些整型开辟好内存空间，当代码中定义该范围的整型时，不会再重新分配内存地址，而是直接引用整型池中的数据。
...
3. 装饰器的应用场景，写一个统计性能的装饰器
   ...
   应用场景有检查是否授权访问某个站点、验证是否登录、日志记录等。
...
4. 一个函数被多个装饰器装饰的时候，装饰器的执行顺序是什么样的？

   首先理解一下Python解释器是如何处理装饰器的
   ...
   所以，当Python解释器执行这段代码的时候，过程如下

   > - 首先调用解释器AA，并且把函数f作为参数传给AA，返回它内部的函数func_a
   > - 所以此后f指向的便是AA内部的函数func_a
   > - 以后在调用函数f的时候实际上就是调用的func_a，传给函数f的参数也被传给了func_a
   > - 然后func_a会再把函数f的参数传递给其内部的func，也即最初真实的函数f
   > - 最后函数执行完了返回的是函数f的返回值，整个过程就像是在直接调用函数f一样

   如果是一个函数被两个装饰器修饰，执行结果又是什么样的呢？
   ...
   解释器会先执行装饰器函数AA，因此打印了AA 1，然后把func_a作为参数传递给装饰器函数BB，因此打印了BB 1，返回BB内部函数func_b给函数名f，此时调用函数f的时候实际上调用的是func_b，因此打印了BB 2，func_b里面的func实际上是func_a，因此会接着调用func_a，故打印了AA 2，func_a里面的func实际上是真实的函数f，因此会调用最初的函数f，打印了F，并且返回了函数执行结果10。

   **总结多个装饰器的执行顺序是：先由内及外，再由外及内**

5. 说一下你对可迭代对象、迭代器和生成器的理解。
...
6. 写一个删除列表中重复元素的函数，要求去重后元素的相对位置保持不变
...
7. 假设你使用的是官方的的CPython，说出下面代码的运行结果。
...
14. 说一下对`hasattr()`，`getattr()`，` setattr()` 函数的理解。

15. 简述一下`any()`和`all()`方法。

16. 说一下Python中的多线程和多进程的应用场景和优缺点。

17. 解释一下线程池的工作原理。

18. 举例说明什么情况下会出现`KeyError`、`TypeError`、`ValueError`。

19. 说一下Python中常见的异常类型。

20. 说一下你对Python中模块和包的理解。

21. 函数参数`*args`和`kwargs`分别代表什么？
